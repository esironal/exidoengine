...
/**
 * Returns radio HTML block
 *
 * @param  array
 * @param  string
 * @return string
 *
  function formBlockRadio(array $data, $class = 'block-field')
  {
    return formBlockField($data, 'radio', $class);
  }

// -------------------------------------------------------------------------------

/**
 * Returns checkbox HTML block
 *
 * @param  array
 * @param  string
 * @return string
 *
  function formBlockCheckbox(array $data, $class = 'block-field')
  {
    return formBlockField($data, 'checkbox', $class);
  }

// -------------------------------------------------------------------------------

/**
 * Returns the HTML block with a submit button.
 * @param array $data
 * @param string $class
 * @return string
 * @deprecated
 */
function formBlockSubmit(array $data, $class = 'form-fieldset fieldset-submit')
{
  $output = '<div';
  if(isset($data['name'])) {
    $output.= ' id="sys-block-'.$data['name'].'"';
  }
  if( ! empty($class)) {
    $output.= ' class="'.$class.'"';
  }
  if( ! isset($data['id']) and isset($data['name'])) {
    $data['id'] = 'sys-sbm-'.$data['name'];
  }
  $output.= ">".EXIDO_EOL;
  $output.= formSubmit($data);
  $output.= "</div>".EXIDO_EOL;
  return $output;
}
...
/**
 * Returns the HTML block with a password input field.
 * @param array $data
 * @param string $class
 * @return string
 * @deprecated
 */
function formBlockPassword(array $data, $class = 'form-fieldset fieldset-pasword')
{
  return formBlockField($data, 'password', $class);
}

// -------------------------------------------------------------------------------

/**
 * Returns the HTML block with an input field.
 * @param array $data
 * @param string $class
 * @return string
 * @deprecated
 */
function formBlockInput(array $data, $class = 'form-fieldset fieldset-input')
{
  return formBlockField($data, 'input', $class);
}

// -------------------------------------------------------------------------------

/**
 * Returns the HTML block with a textarea field.
 * @param array $data
 * @param string $class
 * @return string
 * @deprecated
 */
function formBlockTextarea(array $data, $class = 'form-fieldset fieldset-textarea')
{
  return formBlockField($data, 'textarea', $class);
}

// -------------------------------------------------------------------------------

/**
 * Returns the HTML block with a dropdown selector.
 * @param array $data
 * @param string $class
 * @return string
 * @deprecated
 */
function formBlockDropdown(array $data, $class = 'form-fieldset fieldset-dropdown')
{
  return formBlockField($data, 'dropdown', $class);
}
 ...
 * @deprecated
 */
function formBlockField(array $data, $type, $class = '')
{
  if(empty($class)) {
    $class = 'form-fieldset fieldset-'.$type;
  }
  if(isset($data['label']) and ! empty($data['label'])) {
    $label = $data['label'];
    unset($data['label']);
  }
  if(isset($data['hint']) and ! empty($data['hint'])) {
    $hint = $data['hint'];
    unset($data['hint']);
  }
  if(isset($data['suffix']) and ! empty($data['suffix'])) {
    $suffix = $data['suffix'];
    unset($data['suffix']);
  }
  if(isset($data['prefix']) and ! empty($data['prefix'])) {
    $prefix = $data['prefix'];
    unset($data['prefix']);
  }
  ...
  $output = '<div';
  if(isset($data['name'])) {
    $output.= ' id="sys-div-'.$data['name'].'"';
  }
  $output.= ' class="'.$class.'"'.">".EXIDO_EOL;
  if( ! isset($data['id'])) {
    $data['id'] = 'sys-inp-'.$data['name'];
  }
  if(isset($label)) {
    if(isset($data['label-class'])) {
      $output.= formLabel($label, $data['id'], array(
        'class' => $data['label-class']
      ));
    } else {
      $output.= formLabel($label, $data['id']);
    }
  }

  if(isset($data['disabled']) and $data['disabled'] == true) {
  ...
  if(isset($prefix)) {
    $output.= formHint($prefix, 'field-prefix');
  }
  switch($type) {
    case 'input' :
        if(isset($data['inputPrepend'])) {
          $output.= $data['inputPrepend'];
          unset($data['inputPrepend']);
        }
        $output.= formInput($data);
        if(isset($data['inputAppend'])) {
          $output.= $data['inputAppend'];
          unset($data['inputAppend']);
        }
      break;
    case 'password' :
        $output.= formPassword($data);
      break;
    case 'textarea' :
        $output.= formTextarea($data);
      break;
    case 'dropdown' :
        ...
        $extra = (isset($data['id']))?'id="'.$data['id'].'"':'';
        $extra = $extra.((isset($data['class']))?' class="'.$data['class'].'"':'');
        $extra = $extra.((isset($data['disabled']))?' '.$data['disabled']:'');
        $extra = $extra.((isset($data['onchange']))?' onchange="'.$data['onchange'].'"':'');
        $output.= formDropdown($data['name'], $options, $select, $extra);
      break;
    default:
      return '';
  }
  if(isset($suffix)) {
    $output.= formHint($suffix, 'form-field-suffix');
  }
  if(isset($hint)) {
    $output.= formHint($hint);
  }
  $output.= "</div>".EXIDO_EOL;
  return $output;
}
