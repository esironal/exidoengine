...
final class Exido_Main_Core
{
  ...
  public static function initialize()
  {
    ...
    // Load the logger
    self::$log    = Exido_Log::instance();
    // Load the default configuration files
    self::$config = Exido_Config::instance()->attach(new Exido_Config_File);
    // Load the i18n class
    self::$i18n   = Exido_I18n::instance();

    // Enable debug log
    if(self::$log_debug) {
      self::$log->attach(new Exido_Log_File(APPPATH.'data/log/debug'), array('EXIDO_DEBUG_LOG'));
      self::$log->add('EXIDO_DEBUG_LOG', 'Initialize framework');
    }

    // Enable error log
    if(self::$log_error) {
      self::$log->attach(new Exido_Log_File(APPPATH.'data/log/error'), array('EXIDO_ERROR_LOG'));
    }
    ...
    self::$is_xml = Exido_Main_Input::instance()->isXmlRequest();
