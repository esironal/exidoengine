...
final class Exido_Main_Debug
{
  ...
  {
    return Core_Debug::_dump($value, $length);
  }

  // ---------------------------------------------------------------------------

  /**
   * Helper for Core_Debug::dump(), handles recursion in arrays and objects.
   * @param string $var
   * @param int $length
   * @param int $level
   * @return string
    ...
    elseif(is_array($var))
    {
      ...
      if(empty($var)) {
        ...
        foreach($var as $key => & $val) {
          ...
          $output[] = "$space$s$key => ".Core_Debug::_dump($val, $length, $level + 1);
        }
        unset($var[$marker]);
        $output[] = "$space)</span>";
      } else {
    ...
    elseif(is_object($var))
    {
      ...
      elseif($level < 10)
      {
        ...
        foreach($array as $key => & $val) {
          ...
          $output[] = "$space$s$access $key => ".Core_Debug::_dump($val, $length, $level + 1);
        }
        unset($objects[$hash]);
        $output[] = "$space}</code>";
      }
   ...
   *     echo Core_Debug::source(__FILE__, __LINE__);
   *
   * @param string $file
   ...
   *     echo implode('<br/>', Core_Debug::trace());
   *
   * @param array $trace
   * @return array
   */
  public static function trace(array $trace = null)
  {
    ...
    foreach($trace as $step) {
      ...
      if(isset($step['file']) and isset($step['line'])) {
        ...
        $source = Core_Debug::source($step['file'], $step['line']);
      }
