 ...
 * @version    1.0
 */
abstract class Exido_Controller_Cron_Abstract extends Exido_Controller_Abstract
{
  /**
   * Started job list
   * @var array
      ...
      if($range = ipRangeParser($ip)) {
        if( ! ipCheckRange($ip_block, $range[0], $range[1])) {
          if($range[0] == $range[1]) {
            ...
            print sprintf(__("Your IP %s doesn't match in allowed IP range %s"), $this->input->ip(), $ip);
          }
          return false;
        }
      } else {
          ...
          foreach($job_data['callback'] as $callback) {
            ...
            if($func = explode(':', $callback) and count($func) > 1) {
              ...
              if(function_exists($callback)) {
                ...
                $this->_log[$job_name][$callback]['status'] = true;
                $this->_log[$job_name][$callback]['result'] = sprintf(__('Undefined cron function %s()'), $callback);
              }
            }
          }
        } else {
