...
final class Model_User extends Model_Db_Abstract
{
   ...
   * @return bool
   */
  public function checkIfUsernameIsUnique($user_name, $exclude_user_id = null)
  {
    $q = $this->db->select('user', 'user_id')
      ->where(array('user_name' => $user_name));
    if(is_numeric($exclude_user_id))
      $q->where(array('user_id' => '{!=}'.$exclude_user_id));
    if($r = $q->exec())
   ...
   * @return bool
   */
  public function checkIfEmailIsUnique($user_email, $exclude_user_id = null)
  {
    $q = $this->db->select('user', 'user_id')
